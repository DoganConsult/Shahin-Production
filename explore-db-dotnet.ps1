#!/usr/bin/env pwsh
# Database Explorer using .NET Npgsql (same as application)
# No external tools needed - uses .NET libraries
...
Write-Host "Database Explorer (.NET Edition)" -ForegroundColor Cyan
Write-Host "Using Npgsql - Same as Application" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# Create C# script to explore database
$explorerCode = @'
using System;
using System.Data;
using Npgsql;
...
Set-Content -Path $tempFile -Value $explorerCode

# Build and run with dotnet script
try {
    Write-Host "Building and running database explorer..." -ForegroundColor Cyan
    Write-Host ""

    # Build arguments
    $scriptArgs = @()
    if ($ShowSchema) {
        $scriptArgs += "--show-schema"
    }
    if ($ShowSampleData) {
        $scriptArgs += "--show-sample-data"
    }
    if ($TableName) {
        $scriptArgs += "--table=$TableName"
    }
    if ($SampleRows -ne 5) {
    ...
    dotnet script $tempFile @scriptArgs
...
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  .\explore-db-dotnet.ps1" -ForegroundColor Gray
Write-Host "  .\explore-db-dotnet.ps1 -ShowSchema" -ForegroundColor Gray
Write-Host "  .\explore-db-dotnet.ps1 -ShowSampleData -TableName Tenants" -ForegroundColor Gray
Write-Host ""
