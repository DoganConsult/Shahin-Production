    ...
    public class PasswordHistoryService : IPasswordHistoryService
    {
        ...
        public async Task<bool> IsPasswordInHistoryAsync(string userId, string newPassword, int historyCount = 0)
        {
            ...
            try
            {
                ...
                // Create a dummy user for password verification (convert string ID to Guid)
                var dummyUser = new ApplicationUser();
                if (Guid.TryParse(userId, out var guidId))
                {
                    // Use reflection to set the protected Id property for verification purposes
                    var idProperty = typeof(ApplicationUser).BaseType!.GetProperty("Id");
                    idProperty?.SetValue(dummyUser, guidId);
                }
